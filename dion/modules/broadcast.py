...
from dion.config import SUDO_USERS
from dion.utils.filters import command
from dion.tgcalls import client as USER
from dion.database.chats import add_served_chat, blacklisted_chats, get_served_chats
...
@app.on_message(command("mgcast") & filters.user(SUDO_USERS))
...
@app.on_message(command("mbroadcast") & filters.user(SUDO_USERS) & ~filters.edited)
async def broadcast_message(_, message):
    if len(message.command) < 2:
