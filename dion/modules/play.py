...
from dion.tgcalls import calls, queues
from dion.tgcalls.calls import client as ASS_ACC
...
import dion.tgcalls
from dion.tgcalls import youtube
...
from dion.utils.decorators import errors, sudo_users_only
from dion.utils.administrator import adminsOnly
from dion.utils.errors import DurationLimitError
from dion.utils.gets import get_url, get_file_name
from dion.modules.admins import member_permissions
...
# play
@Client.on_message(
    command(["play", f"play@{BOT_USERNAME}", "dplay", f"dplay@{BOT_USERNAME}", "rplay"])
    & filters.group
    & ~filters.edited
    & ~filters.forwarded
    & ~filters.via_bot
...
async def play(_, message: Message):
    ...
    if audio:
        ...
        title = audio.title
        thumb_name = "https://telegra.ph/file/9a01861c10f09e29191ba.png"
        ...
        file_path = await dion.tgcalls.convert(
        ...
        file_path = await dion.tgcalls.convert(x)
    else:
        if len(message.command) < 2:
